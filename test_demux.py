...
def test_read_config():
    config = demux.read_config("dag_run.conf")
    assert 7 == len(config["sequencers"])

def test_find_completed_runs():
    demux.find_completed_runs(demux.read_config("dag_run.conf"), 30)
    assert 0 == 0
...
def test_create_sequencer_done_file(tmpdir):
    ...
    runs_to_demux = demux.find_completed_runs(sequencers_json,30)
