...
def create_fastq_folders(run_demux_dir):
    ...
    for project in glob.iglob("Project_*"):
        ...
        for fastq in glob.iglob("*fastq.gz"):
            sample_id = re.split("_S(\d+)_L00(.)_[R|I](.)_001.fastq.gz", fastq)[0]
            print("Processing: {}".format(sample_id))
            sample_folder = "Sample_{}".format(sample_id)
            pathlib.Path(sample_folder).mkdir(parents = True, exist_ok = True)
            print(sample_folder)
            # all_fastqs_of_one_sample = sample_name + "_*" + ".fastq.gz"
            move_fastq_file_to_sample_folder = "mv {} {}".format(fastq, sample_folder)
            print(move_fastq_file_to_sample_folder)
            call(move_fastq_file_to_sample_folder, shell = True)
            # Special behavior for 08822 projects, create merged R1 fastq.gz & merged R2 fastq.gz for bwamem2 consumption
            if "_IGO_08822" in sample_folder and not "_RNA_IGO_" in sample_folder:
                ppg_dir = run_demux_dir +"_PPG/" + project + "/" + sample_folder + "/"
                print("Creating PPG fastq dirs: " + ppg_dir)
                os.makedirs(ppg_dir)
                cat_r1_cmd = "cat " + sample_folder + "/*_R1_001.fastq.gz > " + ppg_dir + "/" + sample_id + "_S01_R1_001.fastq.gz"
                print(cat_r1_cmd)
                call(cat_r1_cmd, shell = True)
                cat_r2_cmd = "cat " + sample_folder + "/*_R2_001.fastq.gz > " + ppg_dir + "/" + sample_id + "_S01_R2_001.fastq.gz"
                print(cat_r2_cmd)
