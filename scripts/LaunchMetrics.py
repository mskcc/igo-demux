...
class LaunchMetrics(object):
	...
	def dragen(sample, run, sample_parameters, work_directory, dragen_directory, fastq_list):
		...
		if ("BAITS" in sample_parameters.keys()):
			...
			hs_metrics = "{} CollectHsMetrics --INPUT {}___MD.bam --OUTPUT {}{}___{}___HS.txt --REFERENCE_SEQUENCE {} --BAIT_INTERVALS {} --TARGET_INTERVALS {}".format(PICARD_AND_JAR, sample.sample_id, work_directory, metric_file_prefix, PICARD_VERSION, sample_parameters["REFERENCE"], sample_parameters["BAITS"], sample_parameters["TARGETS"])
