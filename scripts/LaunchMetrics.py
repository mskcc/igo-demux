...
class LaunchMetrics(object):
	...
	def dragen_methylation(sample, run, sample_parameters, work_directory, dragen_directory, fastq_list):
		...
		if (sample_parameters["GTAG"] == "GRCh38") or (sample.genome == "Synthetic"):
			...
			dragen_path = "/igo/work/igo/dragen_hash_tables/4.2/ecoli_GCF_000005845_2_ASM584v2_methylated"
			vcfFileOption = ""
		elif (sample.genome == "J02459_1_bacteriophage_lambda_methylated"):
			dragen_path = "/igo/work/igo/dragen_hash_tables/4.2/J02459_1_bacteriophage_lambda_methylated"
			vcfFileOption = ""
		else:
		...
		bsub_dragen_methylation_parse_dna = "bsub -J {0}{1} -o {0}{1}.out -w \"done({2}{1})\" -cwd \"{3}\" -n8 -M8 {4}".format(dragen_methylation_parse_header, sample.sample_id, dragen_methylation_job_name_header, dragen_directory, dragen_methylation_parse_dna)
		...
		if (sample.recipe == "Methyl_Capture") and (sample.genome == "Human"):
			hs_metrics_job_name_header = "{}___HS_METRICS___".format(run)
			...
			bsub_hs_metrics = "bsub -J {0}{1} -o {0}{1}.out -w \"done({2}{1})\" -cwd \"{3}\" -n8 -M8 {4}".format(hs_metrics_job_name_header, sample.sample_id, dragen_methylation_job_name_header, dragen_directory, hs_metrics)
			print(bsub_hs_metrics)
			call(bsub_hs_metrics, shell = True)
