...
with DAG(
   ...
   def find_runs_completed(ds, **kwargs):
      completed_runs_path = list()

      # Check if this was run with a config parameter specifying the run to demux
      if "sequencer_path" in kwargs:
   ...
   find_completed = PythonOperator(
      task_id='find_runs',
      python_callable=find_runs_completed,
   )
